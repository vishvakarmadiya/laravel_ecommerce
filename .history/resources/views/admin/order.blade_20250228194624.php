...
<body>
    ...
    outline: none;
    background-color: 
    {{ $data->status == 1 ? '#ffcc00' :  /* Placed (Yellow) */
       ($data->status == 2 ? '#00ccff' :  /* Processing (Light Blue) */
       ($data->status == 3 ? '#ff6600' :  /* Shipped (Orange) */
       ($data->status == 4 ? '#33cc33' :  /* Delivered (Green) */
       ($data->status == 5 ? '#ff0000' : '#ffffff')))) }};">
