                            ...
                            @else
                                @foreach(getCart() as $cartItem)
                                    <tr>
                                        <td>
                                            <a href="#">
                                                <img src="{{ asset('images/products/' . $cartItem->image) }}" width="50px" height="50px" alt="Image">
                                            </a>
                                        </td>
                                        ...
                                        <td>
                                            <div class="qty d-flex justify-content-center">
                                                <form action="{{ route('cart_update', $cartItem->id) }}" method="POST" class="d-flex">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" name="quantity" value="{{ $cartItem->quantity - 1 }}" class="btn btn-sm btn-light btn-minus">
                                                        <i class="fa fa-minus"></i>
                                                    </button>
                                                    <input type="text" name="quantity" value="{{ $cartItem->quantity }}" class="form-control text-center mx-2" style="width: 50px;">
                                                    <button type="submit" name="quantity" value="{{ $cartItem->quantity + 1 }}" class="btn btn-sm btn-light btn-plus">
