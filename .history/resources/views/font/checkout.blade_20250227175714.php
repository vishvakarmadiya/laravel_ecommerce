...
<body>
    ...
    <script>
        function payment() {
            ...
            var orderId = "{{ $order->id ?? '' }}"; // Ensure order ID is available
            var amount = "{{ $total * 100 }}"; // Convert amount to paise

            var options = {
                "key": razorpayKey, // Fetch API Key from Laravel config
                "amount": amount, // Amount in paise (₹ to paise conversion)
                "currency": "INR",
                "name": "Kittusweety Collection",
                "description": "Order Payment",
                "image": "https://example.com/your_logo",
                "order_id": orderId, // Order ID from backend
                "callback_url": "{{ route('razorpay.verify') }}", // Payment verification route
                "prefill": {
                ...
                "handler": function(response) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            window.location.href = "{{ route('profile') }}"; // Redirect to profile page
                        } else {
