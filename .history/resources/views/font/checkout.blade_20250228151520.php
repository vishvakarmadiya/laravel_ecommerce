...
<body>
                    ...
                    <form action="{{ route('order_store') }}" method="POST">
                        ...
                        <div class="checkout-payment">
                            ...
                            <div class="payment-methods">
                                @foreach(['Cash on Delivery', 'Razorpay'] as $key => $payment)
    ...
    <script>
        window.location = "{{ route('addcartshow') }}";
    </script>
    @endif
    ...
    <script>
        document.getElementById('place-order-btn').addEventListener('click', function(event) {
            ...
            if (selectedMethod === "Razorpay") {
                event.preventDefault(); // Prevent form submission for Razorpay
                payment();
            }
        });

        function payment() {
            let amount = "{{ isset($total) ? $total * 100 : 0 }}"; // Convert to paise
            let options = {
    ...
    if (paymentId) {
        ...
        alert("Payment failed! Please try again.");
                ...
                "prefill": {
                    ...
                    "contact": "{{ auth()->user()->mobile ?? '9517485106' }}"
                },
                "notes": {
