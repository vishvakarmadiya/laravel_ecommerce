...
✅ 2. Move Local Storage Cart to Laravel Session on Page Load
This ensures that when the user logs in, the Local Storage cart is moved to the Laravel session.

📌 Add this to app.blade.php (or any common layout file)
html
Copy
Edit
<script>
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (cart) {
        fetch("{{ route('store.session.cart') }}", {
            method: "POST",
            headers: {
